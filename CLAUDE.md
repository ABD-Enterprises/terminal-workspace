...
## SureForge (supervised runs)

This repository moved from ORC/sure-forge to SureForge (`ABD-Enterprises/sureforge`) on 2026-09-20 for issue #392.

Each supervised run handles one bounded task. The operator reviews a JSON work contract kept outside the worktree, runs `sureforge run --contract FILE --dry-run`, then reruns with `--yes` in a clean linked worktree on an unprotected branch. The run executes the coder command, runs checks, gets a separate reviewer verdict, moves to `awaiting_human_merge`, then `sureforge deliver --request FILE --dry-run|--yes` creates the commit, push, and pull request.

Agents must not commit, push, switch branches, or stash inside a run. The change must stay within the contract scope. Independent review is required before delivery, and merge stays with a human through branch protection.

Task state lives on the GitHub issue board, not in repo markdown. Local validation is `bash scripts/validate.sh`; the contract check command is a bounded subset because SureForge caps one command at 10 minutes.

Operator-local prompts, wrappers, contracts, and reports live outside the repository and are never committed. The previous ORC `orc <verb>` instructions no longer apply.
