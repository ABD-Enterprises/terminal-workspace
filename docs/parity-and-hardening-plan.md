...
| ID | Task | Effort | Source |
|---|---|---|---|
| P2-DM1 | **Introduce `Identity` entity.** Schema: `{id, label, username, authMethod, privateKeyPath?, keyId?, hasPassphrase, comment, source}`. Migrate existing host records: derive identities by deduplicating `(authMethod, username, privateKeyPath)` tuples, link hosts via `identityId`. Update `HostEditor` to pick an Identity. Update Keychain keying to identity-id-keyed (replaces the per-fingerprint workaround). | L | §2.1, §6.6 — **Batches 1, 2, 3 ✅ landed**. **Batch 4 ✅ deprecation pass landed**: per-host `username` / `authMethod` / `privateKeyPath` / `keyLabel` carry `@deprecated` JSDoc with a removal contract pointed at 0.2.0 (after we have evidence the auto-migration ran successfully on every upgrading install). Actual field removal stays gated by that release calendar; nothing in the runtime reads these fields outside `connections.ts`'s fallback path now. |
| P2-DM2 | **Collapse organization axes.** Pick one: keep `tags` (multi-valued, free-form) + `folders` (hierarchical FK). Drop `host.group` (string) and the dead `EnvironmentRecord` entity. Migrate existing data. Filter bar supports multi-tag AND folder. | L | §2.2, §6.7 — **✅ landed**: dropped the dead `EnvironmentRecord` entity, `useEnvironmentsStore`, `EnvironmentEditor`, and `SidebarEnvironments` (their data model linked hosts by magic string match `host.group === env.name` and renaming the env did not re-link the hosts). Replaced with `SidebarGroups` which derives groups from `host.group` directly and exposes a real bulk-rename via the new `useHostsStore.renameGroup` action plus a "detach all hosts" action via `removeGroup`. Multi-tag-AND-folder filter and a hierarchical folder FK are deferred — they're separate features, not strictly part of the IA collapse. |
| P2-PERSIST | Replace localStorage Zustand persist for hosts/keys/known-hosts/identities/snippets with SQLite via `tauri-plugin-sql`. Browser-mode keeps localStorage as fallback. Add migration path. | L | §2.3, §6.14 — **🟡 deferred (network-blocked)**: `tauri-plugin-sql` (Rust) and `@tauri-apps/plugin-sql` (TS) are not in the offline caches. Design notes follow so a network-enabled session can land it cleanly: <ul><li>Single SQLite file at `app_data_dir/termsnip.db`. Schema versioned via `migrations/` with the plugin's first-class migration pipeline.</li><li>One table per Zustand store: `hosts`, `keys`, `known_hosts`, `identities`, `snippets`. JSON columns for nested fields (`environment`, `tags`, `assignedHostIds`, `targetHostIds`) so we don't have to design a normalized schema upfront — that's a Phase-3 concern.</li><li>Tombstone table `deletions(kind TEXT, id TEXT, deleted_at TIMESTAMP)` replaces the in-memory `VaultDeletionMap`.</li><li>One-time migration on first hydrate: read existing localStorage payloads, write through to SQLite, leave the localStorage entries in place for one release as a rollback safety net (mirrors the P1-S5 / P2-DM1 backward-compat patterns).</li><li>Browser-mode keeps the existing localStorage path verbatim — the SQLite plugin is Tauri-only.</li><li>Wrap all reads/writes in a tiny `lib/persistence.ts` that abstracts "store" / "load" so the existing `persist()` middleware swap is one diff per store.</li></ul> |
| P2-NET | Native ship build drops the Node backend. Browser-only path keeps it but with a "demo / preview only" banner. Tauri owns 100% of SSH/SFTP/forwarding/snippets. | L | §5.1, §6.13 — **🟡 partial landed**: removed the dead `if (isTauriRuntime()) return proxyBackendJson(...)` branches inside `backendFetch` / `backendBinaryFetch` (api.ts) — every native API caller already short-circuits to a `termsnip_*` invokeTauriCommand before reaching the fallback. `termsnip_backend_status` now returns synthetic `{ ok: true }` without contacting the Node backend (Tauri owns the runtime). The proxy commands (`termsnip_proxy_backend_json` / `_binary`) and their TS wrappers are marked `@deprecated` with a 0.2.0 removal contract. **Remaining**: actually delete the proxy commands + `BackendBridge`, ship a "demo / preview only" banner in browser mode, and update `docs/architecture.md` to reflect that the Node backend is no longer part of the native ship. |
| P2-FP | First-connect fingerprint UX: when `requireTrusted` is set and an unknown key is encountered, show a modal with the host fingerprint, algorithm, and "Trust" / "Reject" buttons inline. No more separate `/keys?scanHost=` dance. | M | §3.S-1, §6.1 — **✅ landed**: new `lib/ensure-trusted-host-key.ts` runs the scan + prompt; new `store/fingerprint-trust-prompt-store.ts` mirrors the connection-secret-prompt pattern; new `components/common/FingerprintTrustPrompt.tsx` renders the per-algorithm picker with the SHA-256 fingerprint inline; new `lib/launch-host-session.ts` wraps the gate so HostsPage / AppShell palette / Sidebar pinned-favorites all share one connect-flow choke point. Connect aborts cleanly without orphaning a session tab when the user rejects. 6 vitest cases cover the policy short-circuits and scan-error reasons. |
