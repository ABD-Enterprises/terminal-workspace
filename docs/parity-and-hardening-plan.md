...
| ID | Task | Effort | Source |
|---|---|---|---|
| P2-DM1 | **Introduce `Identity` entity.** Schema: `{id, label, username, authMethod, privateKeyPath?, keyId?, hasPassphrase}`. Migrate existing host records: derive identities by deduplicating `(username, privateKeyPath)` tuples, link hosts via `identityId`. Update `HostEditor` to pick an Identity. Update Keychain keying to fingerprint+identity, not hostId. | L | §2.1, §6.6 |
