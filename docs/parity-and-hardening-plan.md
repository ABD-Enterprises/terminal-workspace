...
| P2-PERSIST | Replace localStorage Zustand persist for hosts/keys/known-hosts/identities/snippets with SQLite via `tauri-plugin-sql`. Browser-mode keeps localStorage as fallback. Add migration path. | L | §2.3, §6.14 — **✅ landed**: native loads `sqlite:termsnip.db` through `tauri-plugin-sql`, runs the versioned migration pipeline, persists the five vault stores through `lib/persistence.ts`, and migrates any existing localStorage payload on first hydrate while leaving localStorage in place for rollback. Browser preview keeps the localStorage path. Vault tombstones now hydrate/write through the `deletions(kind, id, deleted_at)` table in native. |
| P2-NET | Native ship build drops the Node backend. Browser-only path keeps it but with a "demo / preview only" banner. Tauri owns 100% of SSH/SFTP/forwarding/snippets. | L | §5.1, §6.13 — **✅ landed**: removed the Rust `BackendBridge`, generic JSON/binary proxy commands, and Node websocket fallback from the Tauri shell. Renderer proxy wrappers are gone, browser mode shows the preview-only banner, and `docs/architecture.md` now describes the Node backend as browser-preview-only. |
