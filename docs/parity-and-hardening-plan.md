...
| ID | Task | Effort | Source |
|---|---|---|---|
| P2-DM1 | **Introduce `Identity` entity.** Schema: `{id, label, username, authMethod, privateKeyPath?, keyId?, hasPassphrase, comment, source}`. Migrate existing host records: derive identities by deduplicating `(authMethod, username, privateKeyPath)` tuples, link hosts via `identityId`. Update `HostEditor` to pick an Identity. Update Keychain keying to identity-id-keyed (replaces the per-fingerprint workaround). | L | §2.1, §6.6 — **Batch 1 ✅ landed**: schema, store, idempotent migration. **Batch 2 ✅ landed**: Settings panel + HostEditor picker. **Batch 3 ✅ landed**: connections.ts now prefers identity-supplied username/authMethod/privateKeyPath (host fields kept as fallback); new per-identity Keychain service (`com.termsnip.runtime.identity-passphrase`) with three-stage load `identity → fingerprint → host` and forward-migration at each found stage; identity delete GC's the per-identity entry. **Batch 4 pending**: remove deprecated host fields one release later. |
