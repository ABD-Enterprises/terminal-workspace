...
- the Node backend still handles the browser path plus jump-host SSH, SFTP, snippets, and forwarding
- the Tauri shell now proxies backend discovery, JSON/binary API access, direct SSH session lifecycle, native session stream I/O, and Keychain-backed runtime secret persistence
